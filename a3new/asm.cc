    ...
    for (int i = 0; i < tokenLines.size(); i++) {
        ...
        if (tokenLine.size() > 0) {
            ...
            if (tokenType.getKind() == Token::LABEL) {
                if (tokenLine.size() == 3) {
                    ...
                    if (tokenLine.at(1).getKind() == Token::WORD) {
                        ...
                        if (op == "jr" || op == "jalr") {
                            ...
                            string tempString;
                            int64_t instrn;
                            if (tokenLine.back().getKind() == Token::REG) {
                                val = tokenLine.back().toLong();
                            }
                            else if (tokenLine.back().getKind() == Token::ID) {
                                tempString = tokenLine.back().getLexeme();
                                if (symT.find(tempString) != symT.end()) {
                                    val = symT.find(tempString)->second;
                                }
                                else {
                                    throw ScanningFailure("ERROR jr/jalr variable");
                                }
                            }
                            else {
                                throw ScanningFailure("ERROR jr/jalr register");
                            }

                            if (op == "jr") {
                                instrn = (0 << 26) | (val << 21) | 8;
                            }
                            else {
                                instrn = (0 << 26) | (val << 21) | 9;
                            }
                            binaryOutput.push_back(instrn);
                        }
                        else {
                            throw ScanningFailure("ERROR jr/jalr");
                        }
                ...
                if (tokenLine.size() == 2) {
                    string op = tokenLine.at(0).getLexeme();
                    if (op == "jr" || op == "jalr") {
                        int64_t val;
                        string tempString;
                        int64_t instrn;
